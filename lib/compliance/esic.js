 ...
 */
export function calculateESIC(grossWages, isPWD = false) {
  const ceiling = isPWD ? ESIC_CONFIG.WAGE_CEILING_PWD : ESIC_CONFIG.WAGE_CEILING;
  
  if (grossWages > ceiling) {
    return {
      applicable: false,
      reason: `Gross wages ₹${grossWages.toLocaleString()} exceeds ceiling ₹${ceiling.toLocaleString()}`,
  ...
  const dailyWage = grossWages / 26; // Approx 26 working days
